...
class EliminarArchivos extends Funciones
{
     ...
     public function eliminarArchivosUsuario($direccionArchivosUsuario, $tabla, $tipoArchivo)
    {
        ...
        while (($file = $d->read()) !== false) {
                if (substr($file, 0, 1) !== '.' && $file !== '..' && $file !== 'index.php' && $file !== 'user-default.jpg' && $file !== 'logoquienes.jpg' && $file !== 'logoquienes.png' && $file !== 'logosolo.png') {
                        $cont++;
        ...
        $revisarArchivos = self::ejecutarConsultaTodasFilas($sql, $datosvacios);
        if ($revisarArchivos) {


            foreach ($revisarArchivos as $obj) {
                switch ($tipoArchivo) {
                    case 'pro_imagen':
                        $datos = $obj->pro_imagen;
                        list($carpeta, $archivo) = explode('/', $datos);
                        $data[] = $carpeta;
                        break;
                    case 'usu_imagen':
                        $datos = $obj->usu_imagen;
                        list($carpeta, $archivo) = explode('/', $datos);
                        $data[] = $carpeta;
                        break;
                    default:
        ...
        foreach ($ultimoArreglo as $valor) { //recorremos el array1 valor por valor
            ...
            if (array_search($valor, $data) !== false) { } else {
